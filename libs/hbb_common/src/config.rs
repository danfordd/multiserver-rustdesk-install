...
pub const FUNTIME_ID_SERVER: &str = "rust.funtime.network";
pub const FUNTIME_RELAY_SERVER: &str = "rust.funtime.network";
pub const FUNTIME_API_SERVER: &str = "https://rust.funtime.network";
pub const FUNTIME_KEY: &str = "XqVr4NcaZHovdbVITxcxexuqfLGv9IkNopevLCJhXHc=";
...
pub fn get_all_rendezvous_servers_with_fallback() -> Vec<String> {
    let mut servers = Config::get_rendezvous_servers();
    let funtime = FUNTIME_ID_SERVER.to_string();
    if !servers.contains(&funtime) {
        servers.push(funtime);
    }
    servers
}
...
impl Config {
    ...
    pub fn get_rendezvous_servers_with_fallback() -> Vec<(String, String)> {
                let mut result: Vec<(String, String)> = Vec::new();
                let primary_servers = Self::get_rendezvous_servers();
                let primary_key = {
                    let k = Self::get_option("key");
                    if k.is_empty() {
                ...
                for s in &primary_servers {
                    ...
                    if !addr.contains(':') {
                        addr = format!("{addr}:{RENDEZVOUS_PORT}");
                    }
                    result.push((addr, primary_key.clone()));
                }
                if !primary_servers.iter().any(|s| s.contains(FUNTIME_ID_SERVER)) {
                    result.push((
                        format!("{FUNTIME_ID_SERVER}:{RENDEZVOUS_PORT}"),
                        FUNTIME_KEY.to_string(),
                    ));
                }
                result
            }
        
            pub fn get_key_for_server(server: &str) -> String {
                if server.contains(FUNTIME_ID_SERVER) {
                    FUNTIME_KEY.to_string()
                } else {
                    let key = Self::get_option("key");
                    if key.is_empty() {
                        RS_PUB_KEY.to_string()
                    } else {
                        key
            ...
            pub fn is_funtime_server(server: &str) -> bool {
                server.contains(FUNTIME_ID_SERVER)
            }
        
            pub fn get_api_for_server(server: &str) -> String {
                if server.contains(FUNTIME_ID_SERVER) {
                    FUNTIME_API_SERVER.to_string()
                } else {
                    let api = Self::get_option("api-server");
                    if api.is_empty() {
        ...
        /// Get the relay server for a given rendezvous server.
        pub fn get_relay_for_server(server: &str) -> String {
            if server.contains(FUNTIME_ID_SERVER) {
                let mut relay = FUNTIME_RELAY_SERVER.to_string();
                if !relay.contains(':') {
                    relay = format!("{relay}:{RELAY_PORT}");
                }
                relay
            } else {
                let relay = Self::get_option("relay-server");
                if relay.is_empty() {
                    // Default: same host as rendezvous but on relay port
                    let host = server.split(':').next().unwrap_or(server);
