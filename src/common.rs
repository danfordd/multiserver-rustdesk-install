...
#[inline]
pub fn get_api_server(api: String, key: String) -> String {
    if !api.is_empty() {
        return api;
    }

    // If current rendezvous server is funtime, use funtime API
    let rendezvous = Config::get_rendezvous_server();
    if Config::is_funtime_server(&rendezvous) {
        return hbb_common::config::FUNTIME_API_SERVER.to_string();
    }

    // Original logic
    if key.is_empty() || key == hbb_common::config::RS_PUB_KEY {
        "https://admin.rustdesk.com".to_owned()
    } else {
        let s = Config::get_option("custom-rendezvous-server");
        if s.is_empty() {
            format!(
                "https://{}",
                rendezvous.split(':').next().unwrap_or(&rendezvous)
            )
        } else {
            format!("https://{}", s.split(':').next().unwrap_or(&s))
        }
    }
}
