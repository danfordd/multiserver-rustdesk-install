use hbb_common::config::{
    FUNTIME_ID_SERVER, FUNTIME_RELAY_SERVER, FUNTIME_KEY,
};
use std::{
    net::SocketAddr,
    sync::{
...
impl RendezvousMediator {
    ...
    pub async fn start_all() {
        ...
        // Spawn a background task that continuously registers on the funtime
        // fallback server independently of the primary registration loop.
        Self::spawn_funtime_registration_loop();
        ...
        loop {
            ...
            if !Config::get_rendezvous_servers().is_empty() {
                let server_ptr = new_server();
                let servers = Config::get_rendezvous_servers();
                let mut futs = Vec::new();
                for host in servers {
                    let server_ptr = server_ptr.clone();
                    let host = check_port(&host, RENDEZVOUS_PORT);
                    futs.push(tokio::spawn(async move {
                        allow_err!(Self::start(server_ptr, host).await);
                    }));
                }
                join_all(futs).await;
            }
            ...
            let elapsed = conn_start.elapsed().as_millis() as i64;
            if elapsed < REG_INTERVAL {
                sleep((REG_INTERVAL - elapsed) as f32 / 1000.0).await;
            }
        }
    }

    /// Spawns a long-lived background tokio task that periodically registers
    /// this peer on the funtime fallback rendezvous server.  This runs
    /// independently of the primary server loop so both servers always know
    /// about this peer.
    fn spawn_funtime_registration_loop() {
        // Only spawn if the user hasn't already configured funtime as primary.
        let primary_servers = Config::get_rendezvous_servers();
        let already_primary = primary_servers
            .iter()
            .any(|s| s.contains(FUNTIME_ID_SERVER));
        if already_primary {
            log::info!("Funtime server is already a primary server, skipping fallback registration loop");
            return;
        }

        tokio::spawn(async move {
            log::info!(
                "Starting funtime fallback registration loop on {}",
                FUNTIME_ID_SERVER
            );
            loop {
                if SHOULD_EXIT.load(Ordering::SeqCst) {
                    log::info!("Funtime fallback registration loop exiting");
                    break;
                }
                ...
                match Self::funtime_register_once(&id).await {
                    Ok(latency) => {
                        log::debug!(
                            "Funtime fallback registration OK, latency={}ms",
                            latency
                        );
                    }
                    Err(e) => {
                        log::debug!("Funtime fallback registration failed: {}", e);
                    }
                }

                // Re-register at the standard interval
                sleep(REG_INTERVAL as f32 / 1000.0).await;
            }
        });
    }

    /// Perform a single registration cycle on the funtime server using UDP.
    async fn funtime_register_once(id: &str) -> ResultType<i64> {
        let host = check_port(FUNTIME_ID_SERVER, RENDEZVOUS_PORT);
        let tm = Instant::now();

        let (mut socket, addr) = new_udp_for(&host, CONNECT_TIMEOUT).await?;
                    ...
                    match msg_in.union {
                        Some(rendezvous_message::Union::RegisterPeerResponse(rpr)) => {
                            if rpr.request_pk {
                                ...
                                let mut msg_pk = Message::new();
                                msg_pk.set_register_pk(RegisterPk {
                                    id: id.to_owned(),
                                    uuid: uuid.into(),
                                    pk: pk.into(),
                                ...
                                socket.send(&msg_pk, addr).await?;
                                // Wait for RegisterPkResponse
                                if let Ok(Some(Ok((bytes2, _)))) = tokio::time::timeout(
                                    Duration::from_millis(CONNECT_TIMEOUT),
                                    socket.next(),
                                )
                                .await
                                {
                                    if let Ok(msg_in2) = Message::parse_from_bytes(&bytes2) {
                                        if let Some(rendezvous_message::Union::RegisterPkResponse(pkr)) = msg_in2.union {
                                            match pkr.result.enum_value() {
                                                Ok(register_pk_response::Result::OK) => {
                                                    log::info!("Funtime PK registration confirmed");
                                                }
                                                Ok(register_pk_response::Result::UUID_MISMATCH) => {
                                                    log::warn!("Funtime PK registration: UUID mismatch");
                                                }
                                                _ => {
                                                    log::warn!("Funtime PK registration: unknown result");
                                                }
                                            }
                                        }
                                    }
                            ...
                            let latency = tm.elapsed().as_millis() as i64;
                            Ok(latency)
                        }
                        _ => bail!("Unexpected response from funtime server"),
                    }
                } else {
                    bail!("Failed to parse funtime response")
                }
            }
            Ok(Some(Err(e))) => bail!("Funtime socket error: {}", e),
            Ok(None) => bail!("Funtime socket closed"),
            Err(_) => bail!("Funtime registration timeout"),
        }
    }
    ...
    fn get_relay_server(&self, provided_by_rendezvous_server: String) -> String {
        // If this mediator is connected to the funtime server, prefer funtime relay
        if Config::is_funtime_server(&self.host) {
            let mut relay = FUNTIME_RELAY_SERVER.to_string();
            if !relay.contains(':') {
                relay = crate::increase_port(&format!("{}:{}", FUNTIME_RELAY_SERVER, RENDEZVOUS_PORT), 1);
            }
            return relay;
        }
