use hbb_common::config::{
    FUNTIME_ID_SERVER, FUNTIME_KEY, FUNTIME_RELAY_SERVER, RENDEZVOUS_PORT as FUNTIME_RZ_PORT,
};
...
impl Client {
    ...
    async fn _start_inner(
            ...
            let mut feedback = 0;
            // === FUNTIME CHANGE: track which key to use for final connection ===
            let mut effective_key = key.clone();
            // === FUNTIME CHANGE: track if peer was not found on primary ===
            let mut peer_not_found_on_primary = false;
                        ...
                        Some(rendezvous_message::Union::PunchHoleResponse(ph)) => {
                            if ph.socket_addr.is_empty() {
                                ...
                                match ph.failure.enum_value() {
                                    // === FUNTIME CHANGE: don't bail, set flag and break to allow fallback ===
                                    Ok(punch_hole_response::Failure::ID_NOT_EXIST) => {
                                        peer_not_found_on_primary = true;
                                        log::info!("Peer '{}' ID not found on {}", peer, rendezvous_server);
                                        break;
                                    }
                                    Ok(punch_hole_response::Failure::OFFLINE) => {
                                        peer_not_found_on_primary = true;
                                        log::info!("Peer '{}' is offline on {}", peer, rendezvous_server);
                                        break;
                                    }
                                    // === END FUNTIME CHANGE ===
            ...
            // =====================================================================
            // === FUNTIME FALLBACK START ===
            // If peer was not found (ID_NOT_EXIST or OFFLINE) on the primary server
            // and the primary server is NOT already funtime, try funtime as fallback.
            // =====================================================================
            if peer_not_found_on_primary
                && peer_addr.port() == 0
                && !Config::is_funtime_server(&rendezvous_server)
            {
                let funtime_addr = check_port(FUNTIME_ID_SERVER, RENDEZVOUS_PORT);
                log::info!(
                    "Peer '{}' not found on primary '{}', trying funtime fallback '{}'",
                    peer,
                    rendezvous_server,
                    funtime_addr
                );
    
                if let Ok(mut fsocket) = connect_tcp(funtime_addr.clone(), CONNECT_TIMEOUT).await {
                    let funtime_key = FUNTIME_KEY.to_string();
    
                    // Secure the connection to funtime if needed
                    if !funtime_key.is_empty() && !token.is_empty() {
                        if let Err(e) = secure_tcp(&mut fsocket, &funtime_key).await {
                            log::warn!("Failed to secure funtime TCP: {}", e);
                        }
                    }
    
                    // Build PunchHoleRequest with funtime key
                    let mut funtime_msg = RendezvousMessage::new();
                    funtime_msg.set_punch_hole_request(PunchHoleRequest {
                        id: peer.to_owned(),
                        token: token.to_owned(),
                        nat_type: nat_type.into(),
                        licence_key: funtime_key.clone(),
                        conn_type: conn_type.into(),
                        version: crate::VERSION.to_owned(),
                        force_relay: interface.is_force_relay(),
                        ..Default::default()
                    ...
                    for i in 1..=3 {
                        log::info!(
                            "#{} funtime fallback punch attempt, id: {}",
                            i,
                            peer
                        );
                        if let Err(e) = fsocket.send(&funtime_msg).await {
                            log::warn!("Funtime send error: {}", e);
                            break;
                        }
                        if let Some(msg_in) =
                            crate::get_next_nonkeyexchange_msg(&mut fsocket, Some(i * 3000)).await
                        {
                            match msg_in.union {
                                Some(rendezvous_message::Union::PunchHoleResponse(ph)) => {
                                    if ph.socket_addr.is_empty() {
                                        ...
                                        peer_addr = AddrMangle::decode(&ph.socket_addr);
                                        feedback = ph.feedback;
                                        rendezvous_server = funtime_addr.clone();
                                        effective_key = funtime_key.clone();
                                        log::info!(
                                            "Funtime: Hole Punched {} = {}",
                                            peer,
                                            peer_addr
                                        );
                                        break;
                                ...
                                Some(rendezvous_message::Union::RelayResponse(rr)) => {
                                    log::info!(
                                        "Funtime: relay requested, relay_server: {}",
                                        rr.relay_server
                                    );
                                    start = Instant::now();
                                    signed_id_pk = rr.pk().into();
                                    ...
                                    let mut conn = fut.await?;
                                    feedback = rr.feedback;
                                    let typ = if use_ws() { "WebSocket" } else { "Relay" };
                                    log::info!(
                                        "{:?} used to establish funtime {} connection",
                                        start.elapsed(),
                                        typ
                                    );
                                    ...
                                    return Ok((
                                        ...
                                        (feedback, funtime_addr),
                                        false,
                                    ));
                                }
                                _ => {
                                    log::error!(
                                        "Funtime: unexpected protobuf msg: {:?}",
                                        msg_in
                                    );
                                }
                            }
                    ...
                    log::warn!(
                        "Failed to connect to funtime fallback server {}",
                        funtime_addr
                    );
                }
            }
            // === FUNTIME FALLBACK END ===
            // =====================================================================
            ...
            Ok((
                Self::connect(
                    ...
                    my_nat_type,
                    is_local,
                    // === FUNTIME CHANGE: use effective_key instead of key ===
                    &effective_key,
                    &token,
                    conn_type,
                    interface,
